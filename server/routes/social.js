var express = require('express'),
    ...
    Social    = require('../models/social.model'),
...
router.put('/:id', function (req, res, next) {
  Social.findById(({_id: req.params.id}), function (err, item) {
    if (err) {
      return res.status(404).json({
...
router.post('/', function (req, res, next) {
  ...
  delete req.body._id
  var social = new Social(req.body)
//  var social = new Social(req.body)
//   delete social._id
  // console.log(social)
...
router.get('/page/:page', function (req, res, next) {
  ...
  Social.find().count((err, totalItems) => {
    if(err)
      res.send(err);
    else
        Social.aggregate(
...
router.delete('/:id', function (req, res, next) {
  Social.findById((req.params.id), function (err, item) {
    if (err) {
      return res.status(500).json({
